--[[
    Auto Farm Bonds PRO V7 (Community Enhanced – Ultimate Anti-Detect Edition)
    Nâng cấp từ V6: Tích hợp Enemy Avoidance nâng cao, GUI Config, Save/Load Config, cập nhật GUI sau load.
...
local CollectionService = game:GetService("CollectionService") -- Nếu dùng tag enemy
local HttpService = game:GetService("HttpService")
local player = Players.LocalPlayer
local currentCamera = workspace.CurrentCamera

-- Default Config table (thêm bondName vào đây)
local defaultConfig = {
    ...
    pathCheck = true,
    autoReplayDelay = 5,
    enemyWait = 3,         -- Thời gian chờ khi enemy xuất hiện
    safeDistance = 20,     -- Khoảng cách né enemy
    enemyNames = {"enemy", "zombie", "guard", "goblin"}, -- Danh sách tên enemy (không phân biệt hoa thường)
    humanize = {
        movePattern = true,
        clickDelay = {0.1, 0.3},
        failRate = 0.05
...
local spatialGrid = {}
local GRID_SIZE = 100
local replayButtonCache = nil -- Cache for replay button
...
-- Lưu các tham chiếu GUI cần cập nhật
local configSliders = {}
...
local function loadConfig()
    if not isfile or not pcall(function() readfile("BondFarmV7_Config.json") end) then
        config = table.clone(defaultConfig)
        return
    end
    local success, loadedData = pcall(function()
        return HttpService:JSONDecode(readfile("BondFarmV7_Config.json"))
    end)
    if success and type(loadedData) == "table" then
        ...
        config = table.clone(defaultConfig)
        warn("BondFarmV7: Failed to load config. Using defaults.")
    end
end

local function saveConfig()
    if not writefile then
        warn("BondFarmV7: writefile not available. Cannot save config.")
        return
    end
    local success, err = pcall(function()
        writefile("BondFarmV7_Config.json", HttpService:JSONEncode(config))
    end)
    if success then
        ...
        warn("BondFarmV7: Error saving config:", err)
...
-- Hàm cập nhật GUI từ config đã load
...
local function getNearestEnemy()
    local isValid, hrp = validateCharacter()
    if not isValid then return nil, nil end
    local regionSize = Vector3.new(25, 25, 25)
    local region = Region3.new(hrp.Position - regionSize/2, hrp.Position + regionSize/2)
    local parts = workspace:FindPartsInRegion3WithIgnoreList(region, {player.Character}, 30)
    ...
    for _, part in ipairs(parts) do
        ...
        if model and model:FindFirstChildOfClass("Humanoid") then
            ...
            for _, enemyName in ipairs(config.enemyNames) do
                if modelName:find(enemyName:lower()) then
                    local bboxPos = select(1, model:GetBoundingBox()).Position
                    local dist = (hrp.Position - bboxPos).Magnitude
    ...
    return nearestEnemy, minDist
end

local function redirectFromEnemy()
    local isValid, hrp = validateCharacter()
    if not isValid then return false end
    local enemy, enemyDist = getNearestEnemy()
    if not enemy then return false end
    local bboxPos = select(1, enemy:GetBoundingBox()).Position
    local directionAway = (hrp.Position - bboxPos).Unit
    local safeTarget = hrp.Position + directionAway * config.safeDistance
    updateStatus("Redirecting away from enemy...")
    local tween = TweenService:Create(hrp, TweenInfo.new(1, Enum.EasingStyle.Sine), {CFrame = CFrame.new(safeTarget)})
    tween:Play()
    ...
    if path.Status == Enum.PathStatus.Success then
        ...
        for _, waypoint in ipairs(waypoints) do
            if getNearestEnemy() then
                updateStatus("Enemy detected along path, redirecting...")
                redirectFromEnemy()
                return false
            end
...
--#region AUTO FARM FUNCTION (V7)
local function farmBondsV7()
    ...
    -- Kiểm tra enemy trước khi bắt đầu farm
    local enemy, enemyDist = getNearestEnemy()
    if enemy then
        updateStatus("Enemy detected! Redirecting away...")
        if redirectFromEnemy() then return end
    end
    ...
    if not isPathClear(hrp.Position, bond.Position) then
        updateStatus("Path obstructed; using pathfinding...")
    ...
    if getNearestEnemy() then
        updateStatus("Enemy detected before moving, redirecting...")
        if redirectFromEnemy() then return end
    end
    ...
    while true do
        if farming and not paused and math.random(1, 10) == 1 then
            pcall(function()
                ...
                -- Optionally simulate mouse movement; commented for safety.
            end)
        end
...
print("BondFarmV7 Loaded. Config:", config)
updateStatus("Ready")
--#endregion
